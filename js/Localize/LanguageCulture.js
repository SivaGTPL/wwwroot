...
function fnLoadLanguageCultureGrid() {
    ...
    $("#jqgLanguageCulture").GridUnload();
        $("#jqgLanguageCulture").jqGrid({
            url: getBaseURL() + '/Culture/GetLanguageCulture?Culture=' + $("#cboCulture").val() + '&Resource=' + $("#cboResouce").val(),
            datatype: "json",
...
function fnSaveLanguageCulture() {
    ...
    $.ajax({
        url: getBaseURL() + '/Culture/InsertOrUpdateLanguageCulture',
        type: 'POST',
        datatype: 'json',
        data: { obj: r_culture },
        async: false,
...
function fnSaveResourceCulture() {
    ...
    if (resourcekey === '0') {
        toastr.warning("Please Select a Resource Key");
        return;
    }
    ...
    if (IsStringNullorEmpty($("#txtCultureValue").val())) {
        toastr.warning("Please Enter the Culture Value");
        return;
    }
