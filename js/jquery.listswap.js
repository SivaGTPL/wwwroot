(function ($) {
    $.fn.listswap = function (options) {
        ...
        var source_search = '';
        var rand = 1; //Math.floor((Math.random() * 999999) + 1);
        ...
        //        if (this.size() != 2) {
        if (this.length != 2) {
            var warning = '<p style="padding-top:10px;"><i class="fa fa-info"></i> You must choose a <strong>source select list</strong> and a <strong>destination select list</strong> only.</p>';
        ...
        var controls = '<div class="listbox_controls">' +
                            '<ul>' +
            '<li class="add"><button class="mdl-button"><i class="fa fa-plus"></i> &nbsp;' + settings.label_add + '</button></li>' +
                                '<li class="remove"><button class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect  cancel-button"><i class="fa fa-trash-alt"></i> &nbsp;' + settings.label_remove + '</button></li>' +
                            '</ul>' +
                       '</div>';
...
var div_id = "#listboxswap_" + 1;
var source_select_id = "source_4";
var destination_select_id = "destination_4";

$('#listboxswap_1 .source_wrapper .listbox_option').click(function () {
    if ($(this).parent().parent().get(0).className === "source_wrapper") {
        add_remove_handler($(this), div_id + ' .destination_wrapper ul', source_select_id, destination_select_id);
    }
    else {
...
function add_remove_handler(target, destination, select_1, select_2) {
    ...
    $(div_id + " select#" + select_1 + " option[value='" + data_value + "']").remove();
    $(div_id + " select#" + select_2).append($("<option></option>")
        .attr("value", data_value)
        .text(text)
        .attr("data-servicetype", target.attr('data-servicetype'))
        .attr("data-rate", target.attr('data-rate'))
        .attr("data-servicerule", target.attr('data-servicerule'))
        .attr("data-discountpercentage", target.attr('data-discountpercentage'))
        );
    target.remove();
}