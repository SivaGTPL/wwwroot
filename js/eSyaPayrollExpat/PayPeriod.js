...
function fnGridLoadPayPeriod() {
    ...
    $("#jqgPayPeriod").jqGrid({
        url: getBaseURL() + '/PayPeriod/GetPayPeriodsbyBusinessKey?Businesskey=' + $("#cboBusinessLocation").val(),
        mtype: 'Post',
        datatype: 'json',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        jsonReader: { repeatitems: false, root: "rows", page: "page", total: "total", records: "records" },
        colNames: ["Business Key", "Pay Period Date", "Pay Period", "Working Days", "Freeze Payroll", "Post to Finance", "Status", "Actions"],
        colModel: [
...
function fnAddPayPeriod() {
    ...
    if (($("#cboBusinessLocation").val() === '0' || $("#cboBusinessLocation").val() === "")) {
        toastr.warning("Please Select Business Key to add Pay Period");
...
function fnSavePayPeriod() {
    ...
    $("#btnSavePayPeriod").attr('disabled', true);
    $.ajax({
        url: getBaseURL() + '/PayPeriod/InsertOrUpdatePayPeriod',
        type: 'POST',
        datatype: 'json',
