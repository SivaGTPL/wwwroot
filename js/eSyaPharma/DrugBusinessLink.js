...
function fnLoadDrugBusinessTree() {
    ...
    $.ajax({
        url: getBaseURL() + '/DrugInventory/GetGenericDrugList?businessKey=' + $("#cboBusinessLocation").val(),
...
function fnSaveDrugBusinessLink() {
    ...
    var URL;
    URL = getBaseURL() + '/DrugInventory/InsertUpdateDrugBusinessLink';
    $.ajax({
        url: URL,
        type: 'POST',
        datatype: 'json',
        ...
        success: function (response) {
            if (response.Status) {
                ...
                $("#btnSaveDrugBusinessLink").html('Save');
