...
function fnLoadDrugListByVendor() {
    ...
    $("#jqgVendorDrugLink").jqGrid({
        url: getBaseURL() + '/DrugVendor/GetDrugVendorLink?VendorCode=' + $("#cboVendor").val(),
        datatype: 'json',
...
function fnEditVendorDrugLink(e, actiontype) {
    ...
    if (actiontype.trim() == "edit") {
        ...
        $("#btnSaveVendorDrugLink").html(localization.update);
    }
...
function fnSaveVendorDrugLink() {
    ...
    $.ajax({
        url: getBaseURL() + '/DrugVendor/InsertOrUpdateVendorDrugLink',
        type: 'POST',
        datatype: 'json',
        data: { obj },
...
function validateDrugFormulation() {
    ...
    if ($("#txtBusinessShare").val() === "0" || IsStringNullorEmpty($("#txtBusinessShare").val())) {
        toastr.warning("Please enter the Business Share");
        return false;
    }
    if ($("#txtMinimumSupplyQty").val() === "0" || IsStringNullorEmpty($("#txtMinimumSupplyQty").val())) {
        toastr.warning("Please enter the Minimum Supply Quantity");
        return false;
    }
