...
function fnGetDoctorDetails(data) {
    if (data != null) {
        ...
        $.ajax({
            url: getBaseURL() + '/Doctor/GetDoctordetailsbydoctorId?doctorId=' + $('#txtdoctorId').val(),
            type: 'POST',
            datatype: 'json',
...
function fnSaveDoctorDetails() {
    ...
    if (IsStringNullorEmpty($("#txtdoctorId").val())) {
        toastr.warning("Please Doctor to add details");
        return;
    }
    ...
    if (IsStringNullorEmpty($("#txtLanguagesKnown").val())) {
        toastr.warning("Please Enter Language Known");
        return;
    }
    ...
    $.ajax({
        url: getBaseURL() + '/Doctor/InsertOrUpdateIntoDoctordetails',
        type: "POST",
        data: obj,
        dataType: "json",
        contentType: false,
