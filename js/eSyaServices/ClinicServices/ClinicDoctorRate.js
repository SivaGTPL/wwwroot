...
function fnLoadGrid() {
    if ($('#cboBusinessKey').val() != '' && $('#cboDoctor').val() != ''  && $('#cboCurrencyCode').val() != '') {
        fnLoadClinicDoctorRate();
    }

}
function fnLoadClinicDoctorRate() {
    $("#jqgClinicDoctorRate").jqGrid('GridUnload');
    $("#jqgClinicDoctorRate").jqGrid({
        url: getBaseURL() + '/ClinicServices/GetClinicDoctorRateByBKeyDoctorIDCurrCode?businessKey=' + $('#cboBusinessKey').val() + '&doctorid=' + $('#cboDoctor').val() + '&currencycode=' + $('#cboCurrencyCode').val(),
        datatype: 'json',
        mtype: 'GET',
        ...
        onSelectRow: function (rowid) {
            date = $("#jqgClinicDoctorRate").jqGrid('getCell', rowid, "EffectiveDate");
            strdate = date;
            date = moment(date, 'DD-MM-YYYY').toDate();


            $("#dtServiceRateDate").datepicker().datepicker("setDate", date);
            var row = $("#jqgClinicDoctorRate").closest('tr.jqgrow');
        ...
        loadComplete: function (data) {
            ...
            $("#jqgClinicDoctorRate").setGridParam({ datatype: 'json', page: 1 }).trigger('reloadGrid');
            SetGridControlByAction();
        }
    }).jqGrid('navGrid', '#jqpClinicDoctorRate', { add: false, edit: false, search: false, del: false, refresh: false });   
    $("#btnSave").show();
}
...
function fnSaveClinicDoctorRate() {
    ...
    $("#jqgClinicDoctorRate").jqGrid('editCell', 0, 0, false);
    var Clinic_DR = [];
    var id_list = jQuery("#jqgClinicDoctorRate").jqGrid('getDataIDs');
    for (var i = 0; i < id_list.length; i++) {
        var rowId = id_list[i];
        var rowData = jQuery('#jqgClinicDoctorRate').jqGrid('getRowData', rowId);
    ...
    $("#btnSave").attr("disabled", true);
    $.ajax({
        url: getBaseURL() + '/ClinicServices/AddOrUpdateClinicDoctorRate',
        type: 'POST',
        datatype: 'json',
        data: { obj: Clinic_DR },
