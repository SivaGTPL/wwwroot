...
function fnLoadClinicVisitRate() {
    ...
    $("#jqgClinicVisitRate").jqGrid({
        url: getBaseURL() + '/ClinicServices/GetClinicVisitRateByBKeyClinicTypeCurrCodeRateType?businessKey=' + $('#cboBusinessKey').val() + '&clinictype=' + $('#cboClinicType').val() + '&currencycode=' + $('#cboCurrencyCode').val() + '&ratetype=' + $('#cboRateType').val(),
        datatype: 'json',
        mtype: 'GET',
...
function fnSaveClinicVisitRate() {
    ...
    $("#btnSave").attr("disabled", true);
    $.ajax({
        url: getBaseURL() + '/ClinicServices/AddOrUpdateClinicVisitRate',
        type: 'POST',
        datatype: 'json',
        data: { obj: Clinic_VR },
