...
function fnLoadServiceSpecialtyRate() {
    ...
    $("#jqgServiceSpecialtyRate").jqGrid({
        url: getBaseURL() + '/ServiceRates/GetServiceSpecialtyRateByBKeyRateTypeCurrCode?businessKey=' + $('#cboBusinessKey').val() + '&ratetype=' + $('#cboRateType').val() + '&currencycode=' + $('#cboCurrencyCode').val() + '&specialtyId=' + $('#cboSpecialty').val(),
        datatype: 'json',
        mtype: 'GET',
...
function fnSaveServiceSpecialtyRate() {
    ...
    if (IsStringNullorEmpty($("#cboSpecialty").val())) {
        toastr.warning("Please select a Specialty");
        return;
    }
    ...
    $("#btnSave").attr("disabled", true);
    $.ajax({
        url: getBaseURL() + '/ServiceRates/AddOrUpdateServiceSpecialtyRate',
        type: 'POST',
        datatype: 'json',
        data: { obj: Service_BR },
