...
function fnLoadServiceBaseRate() {
    ...
    $("#jqgServiceBaseRate").jqGrid({
        url: getBaseURL() + '/ServiceRates/GetServiceBaseRateByBKeyRateTypeCurrCodeForAdd?businessKey=' + $('#cboBusinessKey').val() + '&ratetype=' + $('#cboRateType').val() + '&currencycode=' + $('#cboCurrencyCode').val(),
        datatype: 'json',
        mtype: 'GET',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        jsonReader: { repeatitems: false, root: "rows", page: "page", total: "total", records: "records" },
        colNames: ["Service ID", localization.ServiceType, localization.ServiceGroup, localization.ServiceClass, localization.ServiceDescription, localization.EffectiveDate, localization.ServiceRule, "", localization.IPBaseRate, localization.IsIprateWardwise,localization.Active],
        ...
        colModel: [
            ...
            { name: "OpbaseRate", width: 50, editable: true, align: 'right', label: localization.OPBaseRate, edittype: 'text' },
            { name: "IpbaseRate", width: 50, editable: true, align: 'right', edittype: 'text' },
            { name: "IsIprateWardwise", editable: true, width: 50, align: 'left', resizable: false, edittype: 'checkbox', formatter: 'checkbox', editoptions: { value: "true:false" } },
    ...
    }).jqGrid('navGrid', '#jqpServiceBaseRate', { add: false, edit: false, search: false, del: false, refresh: false });   
    $("#btnSave").show();
}
...
function fnSaveServiceBaseRate() {
    ...
    $("#btnSave").attr("disabled", true);
    $.ajax({
        url: getBaseURL() + '/ServiceRates/AddServiceBaseRate',
        type: 'POST',
        datatype: 'json',
        data: { obj: Service_BR },
