...
function fnLoadServiceDoctorRate() {
    ...
    $("#jqgServiceDoctorRate").jqGrid({
        url: getBaseURL() + '/ServiceRates/GetServiceDoctorRateByBKeyRateTypeCurrCode?businessKey=' + $('#cboBusinessKey').val() + '&ratetype=' + $('#cboRateType').val() + '&currencycode=' + $('#cboCurrencyCode').val() + '&doctorId=' + $('#cboDoctor').val(),
        datatype: 'json',
        mtype: 'GET',
...
function fnSaveServiceDoctorRate() {
    ...
    $("#btnSave").attr("disabled", true);
    $.ajax({
        url: getBaseURL() + '/ServiceRates/AddOrUpdateServiceDoctorRate',
        type: 'POST',
        datatype: 'json',
        data: { obj: Service_BR },
