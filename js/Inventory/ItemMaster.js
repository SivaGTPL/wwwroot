        ...
        success: function (result) {
            if (result.length > 0) {
                ...
                $("#jqgItemMaster").jqGrid({
                    ...
                    colNames: ["Item Code", "Sku Code","Sku Id", localization.ItemDescription, localization.UnitOfMeasure, "Pack Unit", localization.PackUnit, localization.PackSize, localization.InventoryClass, localization.ItemClass, localization.ItemSource, localization.ItemCriticality, "Barcode ID", localization.Active, localization.Actions],
                    colModel: [
...
function validateItemMaster() {
    ...
    if ($("#txtItemDescription").val().trim().length <= 0) {
        toastr.warning("Please Enter the Item Description");
        $('#txtItemDescription').focus();
        return false;
    }
    if ($("#cboUnitOfMeasure").val() === "0" || $("#cboUnitOfMeasure").val() === "") {
        toastr.warning("Please Select a Unit of Measure");
        $('#cboUnitOfMeasure').focus();
        return false;
    }
    if ($("#cboPackUnit").val() === "0" || $("#cboPackUnit").val() === "") {
        toastr.warning("Please Select a Pack Unit");
        $('#cboPackUnit').focus();
        return false;
    }
    if ($("#cboInventoryClass").val() === "0" || $("#cboInventoryClass").val() === "") {
        toastr.warning("Please Select a Inventory Class");
        $('#cboInventoryClass').focus();
        return false;
    }
    if ($("#cboItemClass").val() === "0" || $("#cboItemClass").val() === "") {
        toastr.warning("Please Select a Item Class");
        $('#cboItemClass').focus();
        return false;
    }
    if ($("#cboItemSource").val() === "0" || $("#cboItemSource").val() === "") {
        toastr.warning("Please Select a Item Source");
        $('#cboItemSource').focus();
        return false;
    }
    if ($("#cboItemCriticality").val() === "0" || $("#cboItemCriticality").val() === "") {
        toastr.warning("Please Select a Item Criticality");
